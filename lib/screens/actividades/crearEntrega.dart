...
class CrearEntrega extends StatefulWidget {
 
  final Actividad actividad;

  const CrearEntrega({Key? key, required this.actividad}) : super(key: key);

  @override
  _CrearEntregaState createState() => _CrearEntregaState();
}

class _CrearEntregaState extends State<CrearEntrega> {
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      final uri = Uri.parse('${servidor.baseURL}api/entregas/create');
      ...
      request.fields['actividad_id'] = widget.actividad.id.toString();
      request.fields['user_id'] = Provider.of<AuthService>(context, listen: false).user.id.toString();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Crear Entrega', style: TextStyle(color: Colors.white)),
